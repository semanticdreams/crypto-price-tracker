...
def main() -> int:
    with sync_playwright() as p:
        ...
        pagination = page.locator("a[href*='?page=']")
        print("pagination links", pagination.count())
        for i in range(min(10, pagination.count())):
            href = pagination.nth(i).get_attribute("href")
            text = pagination.nth(i).inner_text().strip()
            print("page link", i, text, href)

        for page_num in [2, 3]:
            page.goto(f"{URL}?page={page_num}", wait_until="domcontentloaded", timeout=60000)
            page.wait_for_timeout(3000)
            rows = page.locator("table tbody tr")
            print("page", page_num, "rows", rows.count())
            row = rows.filter(has_text="Arbitrum")
            print("page", page_num, "Arbitrum rows", row.count())
            if row.count():
                tds = row.first.locator("td")
                print("page", page_num, "Arbitrum td count", tds.count())
                if tds.count() > 3:
                    print("page", page_num, "Arbitrum price", tds.nth(3).inner_text().strip())
                break
